...
function nut.chat.register(chatType, data)
	...
	if (!data.onChatAdd) then
		...
		data.onChatAdd = function(speaker, text, anonymous)
			...
			if (string.find(data.format, "says") or string.find(data.format, "yells") or string.find(data.format, "whispers")) then
				-- Add capital
				local editCapital = string.sub(text, 1, 1);
				text = (string.upper(editCapital)..string.sub(text, 2, string.len(text)));
			end
		...
		nut.chat.register("ooc", {
			...
			if (!nut.config.get("allowGlobalOOC")) then
				...
				if (delay > 0 and speaker.nutLastOOC) then
					...
					if !(speaker:IsAdmin() or string.find(speaker:getChar():getFlags(), "M")) then
