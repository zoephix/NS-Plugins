...
local PANEL = {}
	...
	function PANEL:Init()
		...
		self:SetPos(ScrW() * 0.2, ScrH() * 0.24)
		...
		for categoryName, commandsTable in pairs(PLUGIN.commands) do
			for k, v in pairs(commandsTable) do
			if (!LocalPlayer():Alive() and !v.allowDead) then
				continue
			end

			if (!table.HasValue(categories, categoryName)) then
				categories[ #categories + 1 ] = categoryName

				categoryCommands = self.list:Add(categoryName)
				categoryCommands:SetExpanded(true)
			end

			if (PLUGIN:IsAllowed(LocalPlayer(), v.group)) then
				local button = categoryCommands:Add("DButton")
